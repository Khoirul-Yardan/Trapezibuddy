...
// ── Task Check (friction mechanic) ──────────────────────────
async function handleTaskCheck(taskId, taskName) {
  // Friction: konfirmasi dulu sebelum mark done
  const confirmed = confirm(`Ketik nama tugas untuk konfirmasi selesai:\n"${taskName}"`)
  if (!confirmed) return
