...
function spawnCharacterExe(charKey) {
  ...
  const dnd = store.get('settings.doNotDisturb', false)
  if (dnd) {
    logger.info('DND mode is active, skipping character spawn.')
    return
  }
...
ipcMain.handle('settings:set', (_, data) => {
  const oldDnd = store.get('settings.doNotDisturb', false)
  store.set('settings', data)
  if (data && data.character) store.set('settings.theme', data.character)
  
  const newDnd = data?.doNotDisturb ?? false
  const charKey = data?.character || store.get('settings.character') || 'agnesTachyon'

  if (newDnd) {
    killAllCharacterExes()
  } else if (oldDnd && !newDnd) {
    // DND turned off, spawn character
    spawnCharacterExe(charKey)
  } else if (data?.character && data.character !== currentRunningChar) {
    // Character changed and not in DND
...
ipcMain.on('theme:apply', (_, theme) => {
  ...
  // Spawn the new character exe immediately
  if (theme !== currentRunningChar) {
    spawnCharacterExe(theme)
  }
...
// ── IPC: Character Control ──────────────────────────────────
ipcMain.on('character:show', (_, charKey) => {
  spawnCharacterExe(charKey)
})

ipcMain.on('character:hide', () => {
  killAllCharacterExes()
})
